...
from .tb_visualize_data import visualize_tb_lightning
...
class PL_MatchAnyEvents(pl.LightningModule):
    def __init__(self, config, pretrained_ckpt=None, profiler=None, dump_dir=None):
        ...
        self.cfg = lower_config(_config['mae'])
        ...
        if pretrained_ckpt:
            ...
            if str(pretrained_ckpt).endswith('.ckpt'):
                state_dict = checkpoint['state_dict']
            elif str(pretrained_ckpt).endswith('.pth'):
    ...
    def _trainval_inference(self, batch):
        ...
        with self.profiler.profile("MAE"):
            with torch.autocast(enabled=self.config.MAE.MP, device_type='cuda'):
                self.matcher(batch)
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            if self.trainer.global_rank == 0 and self.global_step % self.trainer.log_every_n_steps == 0:
                visualize_tb_lightning(self.logger, batch, self.global_step, train=True, scale=self.config.MAE.RESOLUTION[0], num_bins=self.config.NUM_BINS)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        val_plot_interval = 30
        #val_plot_interval = max(self.trainer.num_val_batches[0] // self.n_vals_plot, 50)

        # if batch_idx % val_plot_interval == 0:
        #     visualize_tb_lightning(self.logger, batch, batch_idx, train=False, scale=self.config.MAE.RESOLUTION[0])
    ...
    def validation_epoch_end(self, outputs):
        ...
        for thr in [5, 10, 20]:
            ...
            # print(f'auc@{thr}', torch.tensor(np.mean(multi_val_metrics[f'auc@{thr}'])))
