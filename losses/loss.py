...
class Loss(nn.Module):
    def __init__(self, config):
        ...
        self.loss_config = config['mae']['loss']
        self.match_type = 'dual_softmax'
        self.sparse_spvs = self.config['mae']['match_coarse']['sparse_spvs']
        self.fine_sparse_spvs = self.config['mae']['match_fine']['sparse_spvs']
        ...
        self.overlap_weightc = self.config['mae']['loss']['coarse_overlap_weight']
        self.overlap_weightf = self.config['mae']['loss']['fine_overlap_weight']
        # subpixel-level
    ...
    def forward(self, data):
        ...
        if 'expec_f' not in data:
            ...
            delta = create_meshgrid(patch_size, patch_size, True, sim_matrix_f.device).to(torch.long) # [1, 3, 3, 2]
            m_ids = m_ids[...,None,None].expand(-1, patch_size, patch_size)
            i_ids = i_ids[...,None,None].expand(-1, patch_size, patch_size)
            # Note that j_ids_di & j_ids_dj in (i, j) format while delta in (x, y) format
            j_ids_di = j_ids_di[...,None,None].expand(-1, patch_size, patch_size) + delta[None, ..., 1]
            j_ids_dj = j_ids_dj[...,None,None].expand(-1, patch_size, patch_size) + delta[None, ..., 0]

            sim_matrix_f = sim_matrix_f.reshape(-1, self.local_regressw*self.local_regressw, self.local_regressw+patch_size//2*2, self.local_regressw+patch_size//2*2) # [M, WW, W+2, W+2]
            sim_matrix_f = sim_matrix_f[m_ids, i_ids, j_ids_di, j_ids_dj]
            sim_matrix_f = sim_matrix_f.reshape(-1, patch_size**2)

            sim_matrix_f = F.softmax(sim_matrix_f / self.local_regress_temperature, dim=-1)
            heatmap = sim_matrix_f.reshape(-1, patch_size, patch_size)
