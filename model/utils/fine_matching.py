...
class FineMatching(nn.Module):
    ...
    def get_fine_ds_match(self, conf_matrix, data):
        ...
        # idx_l = torch.tensor([(W//2 * 16) + W//2]).to(conf_matrix.device)
        # val, idx = torch.max(conf_matrix, dim = -1)
        # idx_r = idx[:, idx_l]
        # idx_l = idx_l.repeat(m)[:,None]
        ...
        if self.fp16:
            grid = create_meshgrid(W, W, False, conf_matrix.device).to(torch.float16) - W // 2 + 0.5 # kornia >= 0.5.1
        else:
