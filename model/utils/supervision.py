...
from .geometry import warp_kpts, homography_points
...
def spvs_coarse(data, config):
    ...
    N, _, H0, W0 = data['image'].shape
    _, _, H1, W1 = data['event'].shape
    scale = config['LOFTR']['RESOLUTION'][0]
    scale0 = scale * data['scale0'][:, None] if 'scale0' in data else scale
    scale1 = scale * data['scale1'][:, None] if 'scale1' in data else scale
    h0, w0, h1, w1 = map(lambda x: x // scale, [H0, W0, H1, W1]) # Devided by scale 
    ...
    if 'H' in data:
        grid_pt0_i = homography_points(data['H_inv'], grid_pt0_i)
        grid_pt0_i[out_bound_mask(grid_pt0_i, W0, H0)] = 0.0
    _, w_pt0_i = warp_kpts(grid_pt0_i, data['depth0'], data['depth1'], data['T_0to1'], data['K0'], data['K1'])
    _, w_pt1_i = warp_kpts(grid_pt1_i, data['depth1'], data['depth0'], data['T_1to0'], data['K1'], data['K0'])
    if 'H' in data:
        w_pt1_i = homography_points(data['H'], w_pt1_i)
    ...
    correct_0to1[:, 0] = 0  # ignore the top-left corner
    ...
    if len(b_ids) == 0:
        loguru_logger.warning(f"No groundtruth coarse match found for")
...
def spvs_fine(data, config, logger = None):
    ...
    if m == 0:  # special case: there is no coarse gt
        ...
        grid_pt0_f_unfold = scalei0 * grid_pt0_f_unfold  # [m, ww, 2]
        ...
        for b in range(N):
            ...
            match = int(mask.sum())
            if 'H' in data:
                homo_grid_pt0_f_unfold = homography_points(data['H_inv'][b:b+1], grid_pt0_f_unfold[mask].reshape(1,-1,2))
                
                homo_grid_pt0_f_unfold[out_bound_mask(homo_grid_pt0_f_unfold, W0, H0)] = 0.0
            else:
                homo_grid_pt0_f_unfold = grid_pt0_f_unfold[mask].reshape(1,-1,2)
            correct_0to1_f_mask, w_pt0_i_mask = warp_kpts(homo_grid_pt0_f_unfold, data['depth0'][[b],...],
        ...
        delta_w_pt0_f = delta_w_pt0_i / scalei1 + W // 2 - 0.5
