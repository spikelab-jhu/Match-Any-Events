...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features_list(self, x_list: List[Tensor], masks_list: List[Tensor], layers_taken = [2,5,8,11]) -> List[Dict[str, Tensor]]:
        x = []
        rope = []
        B, w0, h0, _ = x_list[0].shape
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in layers_taken:
                intermediate_output.append([self.norm(feat[:,1:].reshape(B, w0, h0, -1)).permute(0, 3, 1, 2).contiguous() for feat in x])
        # intermediate_output = list(map(list, zip(*intermediate_output)))
        all_x = x
        output = []
            ...
            output.append(
                {
                    ...
                    "x_prenorm": x,
                    "masks": masks,
                    "intermediate":[stage[idx] for stage in intermediate_output]
                }
            )
        return output
